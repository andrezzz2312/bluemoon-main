	...
	useEffect(() => {
		...
		if (canvasRef.current && state) {
			...
			function init() {
				...
				scene.add(estrellas)
				//textura
				const textura = new THREE.TextureLoader(loadingManager).load(
				...
				var m_1 = new THREE.Vector3(
					-0.6073225261727431,
					0.2890031902501969,
					-0.7400246652837772
				)
				var m_2 = new THREE.Vector3(
					-0.7707801703924689,
					0.6325620751447861,
					-0.07591541357510112
				)
				var m_3 = new THREE.Vector3(
					-0.9904384997938112,
					-0.09754758544036858,
					-0.0975502265550357
				)
				var m_4 = new THREE.Vector3(
					0.607322417164568,
					0.2890037594414522,
					0.7400245324569269
				)
          ...
          LightBeam_5.traverse(function(child){
            if(child.isMesh){
              child.material.emissiveIntensity = 20;
				...
				//meteoritos glb
				new GLTFLoader(loadingManager).load(AsteroidsGLB, function (gltf) {
				...
				//bluemoon
				new GLTFLoader(loadingManager).load(BlueMoonGLB, function (gltf) {
					model = gltf.scene
					//anillos de luz
					model.traverse(function (child) {
						if (child.isMesh) {
							...
							poin = new THREE.Mesh(
								new THREE.CircleGeometry(3.1, 30),
							...
							model.add(poin, poin_1, poin_2, poin_3, poin_4)
